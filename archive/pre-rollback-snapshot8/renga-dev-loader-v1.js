(function () {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (prev && prev.tagName.startsWith('URL:')) {
            dnaId = prev.getAttribute('id') || "FOUND";
        }
        ...
        label.innerHTML = `<span>${tagName}${id}${firstClass}</span> ${dnaId ? `<span style="background: #000; color: #fff; padding: 2px 5px; border-radius: 4px; font-size: 9px; margin: 0 5px;">ID ${dnaId}</span>` : ''} <span class="comp-tag">${componentName}</span> <span class="version-tag">${componentVer}</span>`;
    ...
    document.addEventListener('click', (e) => {
        if (e.ctrlKey || e.metaKey) {
            ...
            // Check for DNA Marker
            let prefix = "";
            const prev = lastAtomicElement.previousElementSibling;
            if (prev && prev.tagName.startsWith('URL:')) {
                prefix = prev.outerHTML + "\n";
            }

            const html = prefix + lastAtomicElement.outerHTML;
