/**
 * 🧬 COMPONENT: CRM Detail Panel v3.1
 * Goal: ULTRA HIGH VISIBILITY, BIG TEXT, DARK CONTRAST
 */
        ...
        <div style={{ maxWidth: '1300px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 400px', gap: '40px', animation: 'fadeIn 0.3s' }}>
            ...
            <div style={{ backgroundColor: '#1a1a1f', borderRadius: '32px', border: '1px solid #333', overflow: 'hidden', boxShadow: '0 30px 60px rgba(0,0,0,0.5)' }}>
                ...
                <div style={{ padding: '50px', backgroundColor: '#000', borderBottom: '1px solid #333' }}>
                    ...
                    <p style={{ color: '#E6007E', margin: '5px 0 0 0', fontWeight: 'bold', fontSize: '1rem', letterSpacing: '4px' }}>IDENTIFICATIVO: {item.id.slice(0, 8)}</p>
                </div>
                    ...
                    {activeTab === 'bio' ? (
                            ...
                            <div>
                                ...
                                {editingField === 'pilot_bio' ? (
                                    <>
                                        ...
                                        <button onClick={() => saveField('pilot_bio')} style={btnYellowFull}>SALVA BIOGRAFIA</button>
                                    </>
                                ) : (
                                    <div onClick={() => { setEditingField('pilot_bio'); setTempValue(localItem.pilot_bio || ''); }} style={displayBioBox}>
                                        {localItem.pilot_bio || 'Nessuna biografia inserita. Clicca per aggiungere.'}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '30px' }}>
                            {(type === 'registration' ? regTabs.find(t => t.id === activeTab).fields : ['message']).map(f => (
                                <div key={f}>
                                    <label style={megaLabel}>{f.replace(/_/g, ' ')}</label>
            ...
            <div style={{ backgroundColor: '#0d0d12', borderRadius: '32px', padding: '40px', border: '1px solid #E6007E', display: 'flex', flexDirection: 'column' }}>
                <h3 style={{ color: '#E6007E', fontWeight: 900, letterSpacing: '2px', marginBottom: '30px', fontSize: '1.2rem' }}>CRM ACTIVITY LOG</h3>
                <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '30px' }}>
                    ...
                    {notes.length === 0 && <p style={{ color: '#444', textAlign: 'center' }}>Nessuna attività.</p>}
                </div>
                <div>
                    <textarea value={newNote} onChange={e => setNewNote(e.target.value)} style={darkInputFull} placeholder="Scrivi una nota..." />
...
const displayBox = { backgroundColor: '#26262e', padding: '20px', borderRadius: '16px', border: '1px solid #333', position: 'relative', cursor: 'pointer', minHeight: '60px' };
...
const ultraArea = { width: '100%', backgroundColor: '#000', border: '2px solid #FFCC00', color: '#fff', fontSize: '1.2rem', padding: '20px', borderRadius: '16px', outline: 'none', minHeight: '300px', marginBottom: '20px' };
const displayBioBox = { backgroundColor: '#26262e', border: '1px solid #333', padding: '30px', borderRadius: '24px', fontSize: '1.4rem', color: '#fff', lineHeight: '1.6', cursor: 'pointer' };
const btnSave = { backgroundColor: '#FFCC00', color: '#000', border: 'none', borderRadius: '8px', fontWeight: 900, px: '15px', cursor: 'pointer' };
const btnCancel = { backgroundColor: '#333', color: '#fff', border: 'none', borderRadius: '8px', px: '15px', cursor: 'pointer' };
...
const photoBigBox = { width: '220px', height: '220px', backgroundColor: '#000', border: '3px dashed #FFCC00', borderRadius: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem', position: 'relative', overflow: 'hidden' };
